...
const SUPPORTED_3D = [".stl", ".obj", ".fbx", ".gltf", ".glb", ".3mf", ".step", ".stp"];
const SUPPORTED_IMAGE = [".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"];
const SUPPORTED_DOC = [".pdf", ".md", ".markdown", ".txt"];
const SUPPORTED_FORMATS = [...SUPPORTED_3D, ...SUPPORTED_IMAGE, ...SUPPORTED_DOC];

function getFileType(name: string): "3d" | "image" | "pdf" | "markdown" | "unknown" {
...
export function ModelVersionManager({
  ...
  const has3DFile = uploadedFiles.some((f) => getFileType(f.name) === "3d");
  const first3DFile = uploadedFiles.find((f) => getFileType(f.name) === "3d");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const thumbnailFile = uploadedFiles.find((f) => f.isThumbnail && getFileType(f.name) === "image");
      if (thumbnailFile) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6 py-4">
      ...
      {uploadedFiles.length > 0 && (
        <FileQueue
          ...
          supportedImageFormats={SUPPORTED_IMAGE}
          maxFiles={MAX_FILES_PER_UPLOAD}
