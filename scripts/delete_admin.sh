...
ADMIN_USERS=$(podman exec $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -t -c 'SELECT "Id", "Username", "Email" FROM "Users" WHERE "IsAdmin" = true;')
...
if [ -z "$ADMIN_USERS" ]; then
  ...
  echo "$ADMIN_USERS" | while read -r user_id username email; do
    ...
    podman exec $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -c "DELETE FROM \"RefreshTokens\" WHERE \"UserId\" = '$user_id';"
    
    # Delete user
    podman exec $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -c "DELETE FROM \"Users\" WHERE \"Id\" = '$user_id';"
...
podman exec $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -c 'DELETE FROM "SystemSetups";'
